...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
  	...
  	for (int hor = 0; hor < length; hor++)
    {
      ...
      {
        newHor.push_back(belief_per_cell);
      }
      newGrid.push_back(newHor);
    }
    return newGrid;
}
  ...
  float blurring) 
{
  ...
  // your code here
  int length = beliefs.size();
  int base = beliefs[0].size();
  int new_i, new_j;
  ...
  for (int hor = 0; hor < length; hor++)
  {
    for (int ver = 0; ver < base; ver++)
    {
      new_i = (hor + dy + base) % base;
      new_j = (ver + dx + length) % length;
      newGrid[new_i][new_j] = beliefs[hor][ver];
    }
  }
