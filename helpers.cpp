...
vector< vector<float> > normalize(vector< vector <float> > grid) {
  ...
  for (int hor = 0; hor < grid.size(); hor++) {
    for (int ver = 0; ver < grid[0].size(); ver++) {
      total += grid[hor][ver];
    }
  }
  for (int hor = 0; hor < grid.size(); hor++) {
    for (int ver = 0; ver < grid[0].size(); ver++) {
      newGrid[hor][ver] = grid[hor][ver] / total;
    }
  }
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
  ...
  vector <float> newRow;
  vector < vector <float> > zeros (int length, int base);
  
    // your code here
  int length = grid.size();
  int base = grid[0].size();
  float center_prob = 1.0 - blurring;
  float corner_prob = blurring / 12.0;
  float adjacent_prob = blurring / 6.0;
  
  vector <int> MRX = {-1, 0, 1};
  vector <int> MRY = {-1, 0, 1};
  ...
  newGrid = zeros (length, base);
  
  int new_i, new_j;
  int mrx, mry;
  for (int hor = 0; hor < length; hor++)
  {
    for (int ver = 0; ver < base; ver++)
    {
      val_new = grid[hor][ver];
      for (int m = 0; m < 3; m++)
      {
        mry = MRY[m];
        for(int r = 0; r < 3; r++)
        {
          mrx = MRX[r];
          new_i = (hor + mry + length) % length;
          new_j = (ver + mrx + base) % base;
          mult = window[m][r];
          newGrid[new_i][new_j] += mult * val_new;
        }
      }
    }
  }
